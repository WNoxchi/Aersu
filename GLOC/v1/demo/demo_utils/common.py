# 2018-May-09 21:56 # NOTE: some signficant changes were made to the
# fizyr/keras-retinanet/ repository and their pretrained models, requiring a
# rework of how detection is done.
...
def detect(image, threshold=0.5, mode='ss', fname='', model=None, quiet=True):
    ...
    # predicted_labels = np.argmax(detections[0,:])
    ...
    if mode.lower()=='ss':
        ...
        n = 5
        import pdb; pdb.set_trace()
        for idx, score in enumerate(scores[0,:5]):
            # get bounding box
        ...
        b = boxes[0, inp-1, :4].astype(int)

    # UnSupervised/Automatic Mode
    # automatically pull highest-confidence bounding box
    else:
        # import pdb; pdb.set_trace()
        if not quiet: print(scores[0,0])
        if scores[0,0] >= threshold:
            b = boxes[0, 0, :4].astype(int)
        else:
            return 0
